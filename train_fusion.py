...
def parseArgs():
    ...
    parser.add_argument('-rf_dir', '--rf-data-dir', default="/share2/data/zhouwenqing/UCLA-rPPG/rf_files", type=str,
                        help="Parent directory containing the folders with rf data")
...
def train_model(args, model, datasets):
    ...
    for epoch in range(epoch_start, epochs+1):
        ...
        for batch, (rf_sig, rgb_sig, ppg_sig) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            
            model.train()
            
            rf_sig = rf_sig.type(torch.float32)/1.255e5
            rf_sig = rf_sig.to(args.device)
            ...
            pred_signal = model(rgb_sig, rf_sig)
        ...
        maes_val, rmses_val, pccs_val, (_, _) = eval_model(video_list=datasets["val"].video_list, rf_file_list=datasets["val"].rf_file_list, model=model, device=args.device)
        # maes_train, rmses_train, pccs_train, (_, _) = eval_model(video_list=datasets["train"].video_list, rf_file_list=datasets["train"].rf_file_list, model=model, device=args.device)
...
def main(args):
    ...
    rf_train_files = [i[2:] for i in rgb_train_files]
    rgb_val_files = files_in_fold[args.fold]["val"]
    rf_val_files = [i[2:] for i in rgb_val_files]
    rgb_test_files = files_in_fold[args.fold]["test"]
    rf_test_files = [i[2:] for i in rgb_test_files]
    
    # print(rgb_val_files[:10])
    # print(rf_val_files[:10])
    ...
    # print(f"len(rf_train_files): {len(rf_train_files)} len(rf_val_files): {len(rf_val_files)} len(rf_test_files): {len(rf_test_files)}")
    
    # Dataset
    train_dataset = FusionDataset(rgb_datapath=rgb_folder, rf_datapath=rf_folder,
                                    video_list=rgb_train_files, rf_file_list=rf_train_files)
    val_dataset = FusionDataset(rgb_datapath=rgb_folder, rf_datapath=rf_folder,
                                    video_list=rgb_val_files, rf_file_list=rf_val_files)
    test_dataset = FusionDataset(rgb_datapath=rgb_folder, rf_datapath=rf_folder,
                                    video_list=rgb_test_files, rf_file_list=rf_test_files)
    ...
    """
        ...
        freq_size为超参数, 对应FusionDataset当中的samples参数, 两者必须保持一致
        time_length为超参数, 对应FusionDataset当中的frame_length参数, 两者必须保持一致
    """
    model = FusionModel(frame_depth=4, img_size=128, freq_size=256, time_length=128).to(args.device)
