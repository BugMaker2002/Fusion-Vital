...
from utils.eval import eval_performance_bias, eval_clinical_performance, eval_model
...
def parseArgs():
    ...
    parser.add_argument('-dir', '--data-dir', default="/share2/data/zhouwenqing/UCLA-rPPG/rgb_files", type=str,
    ...
    parser.add_argument('-ckpt','--checkpoint-path', type=str,
                        default="/share1/home/zhouwenqing/Fusion-Vital/ckpt/fusion/best.pth",
...
def main(args):
    ...
    rf_file_list = [i[2:] for i in v_test]
    remove_folders = []
    ...
    model = FusionModel(frame_depth=4, img_size=128, time_length=64).to(args.device)
    model.load_state_dict(torch.load(ckpt_path))

    maes_val, rmses_val, pccs_val, hr_test = eval_model(video_list=v_test, rf_file_list=rf_file_list, model=model, device=args.device)
