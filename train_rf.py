...
def train_model(args, model, datasets):
    ...
    for epoch in range(epoch_start, epochs+1):
        ...
        for batch, (rf_sig, _, ppg_sig) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        ...
        maes_val, rmses_val, pccs_val, (_, _) = eval_rf_model(rf_file_list=datasets["val"].rf_file_list, model=model, device=args.device)
...
def main(args):
    ...
    """
    model = RFModel(freq_size=256, time_length=128).to(args.device)
