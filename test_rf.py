...
from utils.eval import eval_performance_bias, eval_clinical_performance, eval_rf_model
...
def parseArgs():
    ...
    parser.add_argument('-ckpt','--checkpoint-path', type=str,
                        default="/share1/home/zhouwenqing/Fusion-Vital/ckpt/rf/best.pth",
...
def main(args):
    ...
    model = RFModel(freq_size=256, time_length=64).to(args.device)
    model.load_state_dict(torch.load(ckpt_path, map_location=args.device))

    maes_val, rmses_val, pccs_val, hr_test = eval_rf_model(rf_file_list=rf_file_list, model=model, device=args.device)
